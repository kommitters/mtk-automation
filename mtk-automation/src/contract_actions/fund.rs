...
pub(crate) fn set_available_funds_to_issue(env: &Env, new_value: &i128) {
    env.storage().set(&DataKey::AllowedF, new_value);
}

pub(crate) fn get_available_funds_to_issue(env: &Env) -> i128 {
    env.storage().get(&DataKey::AllowedF).unwrap().unwrap()
}

pub(crate) fn get_contract_balance(env: &Env) -> i128 {
